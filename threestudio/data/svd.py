...
@dataclass
class SVDImageDataModuleConfig:
...
class SVDImageDataBase:
    def setup(self, cfg, split):
        self.split = split
        self.rank = get_rank()
        self.cfg: SVDImageDataModuleConfig = cfg
    ...
    def load_images(self):
        ...
        print(
            f"[INFO] svd image dataset: load image {self.cfg.image_path} {self.rgb.shape}"
        )
        self.depth = None
        self.normal = None
...
class SVDImageIterableDataset(IterableDataset, SVDImageDataBase, Updateable):
...
class SVDImageDataset(Dataset, SVDImageDataBase):
    def __init__(self, cfg: Any, split: str) -> None:
        super().__init__()
        self.setup(cfg, split)
...
class SVDImageDataModule(pl.LightningDataModule):
    cfg: SVDImageDataModuleConfig

    def __init__(self, cfg: Optional[Union[dict, DictConfig]] = None) -> None:
        super().__init__()
        self.cfg = parse_structured(SVDImageDataModuleConfig, cfg)

    def setup(self, stage=None) -> None:
        if stage in [None, "fit"]:
            self.train_dataset = SVDImageIterableDataset(self.cfg, "train")
        if stage in [None, "fit", "validate"]:
            self.val_dataset = SVDImageDataset(self.cfg, "val")
        if stage in [None, "test", "predict"]:
            self.test_dataset = SVDImageDataset(self.cfg, "test")
