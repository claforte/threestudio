...
exp_name0=GSO_hashgrid_drunk_gt_no_sds
exp_name=GSO_hashgrid_drunk_gt_no_sds_refine
...
do (
    python launch.py --train --config=$config0 name=$exp_name0 tag=$(basename -- $dir) data.train_on_gt=True \
    data.sober_or_drunk=$sober_or_drunk data.omnidata_normal_path=$mono_normal_dir/$(basename -- $dir)/mono_normal/ && \

    python launch.py --train --config=$config data.train_on_gt=True \
