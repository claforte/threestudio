...
exp_name0=GSO_hashgrid_drunk_sds
exp_name=GSO_hashgrid_drunk_sds_refine
config0=configs/kplanes_sds_mark.yaml
config=configs/kplanes_sds_refine_mark.yaml
sober_or_drunk=drunk
mono_normal_dir="/weka/home-chunhanyao/GSO_mark/GSO_DRUNK_21"

for dir in ../GSO_sober21/*/; \
do (
    CUDA_VISIBLE_DEVICES=0 python launch.py --train --config=$config0 name=$exp_name0 tag=$(basename -- $dir) \
    data.sober_or_drunk=$sober_or_drunk data.omnidata_normal_path=$mono_normal_dir/$(basename -- $dir)/mono_normal/ && \

    CUDA_VISIBLE_DEVICES=0 python launch.py --train --config=$config \
    ...
    # CUDA_VISIBLE_DEVICES=0 python launch.py --test --config=configs/$config \
    # resume=outputs/$exp_name/$(basename -- $dir)/ckpts/last.ckpt \
